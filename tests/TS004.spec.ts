...
/**
 * This test suite is to validate the form error redirection and sidebar error number for missing input fields form submission
 */

test.describe('Handle missing input field submission', () => {
  let pm: PageManager;
  ...
  test('Navigate into all menu sections via menu bar', async () => {
    await pm.getMenu().navigateIntoContactDetails()
    await pm.getMenu().navigateIntoProposal()
    await pm.getMenu().navigateIntoBusinessImpact()
    await pm.getMenu().navigateIntoCost()
    await pm.getMenu().navigateIntoDeclareAndReview()
  })

  test('Validate form error redirection and sidebar error number for missing form submission', async () => {
    await pm.getDeclarePage().verifyFormErrorRedirectionAndSidebarErrorNumber()
  })
